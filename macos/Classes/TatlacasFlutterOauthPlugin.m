...
- (void)handleAuthorizeMethodCall:(NSDictionary *)arguments result:(FlutterResult)result exchangeCode:(BOOL)exchangeCode {
    ...
    if (requestParameters.serviceConfigurationParameters != nil) {
        ...
        [self performAuthorization:serviceConfiguration
        ...
        [OIDAuthorizationService discoverServiceConfigurationForDiscoveryURL:discoveryUrl
                                                                  completion:^(OIDServiceConfiguration *_Nullable configuration,
                                                                      ...
                                                                      [self performAuthorization:configuration
                                                                                        clientId:requestParameters.clientId
                                                                                    clientSecret:requestParameters.clientSecret
                                                                                          scopes:requestParameters.scopes redirectUrl:requestParameters.redirectUrl
                                                                            additionalParameters:requestParameters.additionalParameters
                                                                          preferEphemeralSession:requestParameters.preferEphemeralSession
                                                                                          result:result exchangeCode:exchangeCode];
                                                                  }];
    } else {
...
- (void)performAuthorization:(OIDServiceConfiguration *)serviceConfiguration
    ...
    NSURL *successURL = [NSURL URLWithString:redirectUrl];
    ...
    if (exchangeCode) {
        _redirectHTTPHandler.currentAuthorizationFlow = [OIDAuthState
                authStateByPresentingAuthorizationRequest:request
                                                 callback:^(OIDAuthState *_Nullable authState,
                                                     ...
                                                     if (authState) {
                                                         ...
                                                         [self finishWithError:AUTHORIZE_AND_EXCHANGE_CODE_ERROR_CODE
                                                                       message:[self formatMessageWithError:AUTHORIZE_ERROR_MESSAGE_FORMAT
                                                                                                      error:error]
                                                                        result:result];
                                                     }
                                                 }];
                                                                       ...
                                                                       externalUserAgent:agent
                                                                                callback:^(OIDAuthorizationResponse *_Nullable authorizationResponse, NSError *_Nullable error) {
                                                                                    if (authorizationResponse) {
                                                                                        ...
                                                                                        processedResponse[@"authorizationAdditionalParameters"] = authorizationResponse.additionalParameters;
                                                                                        processedResponse[@"authorizationCode"] = authorizationResponse.authorizationCode;
                                                                                        processedResponse[@"codeVerifier"] = authorizationResponse.request.codeVerifier;
...
- (void)performTokenRequest:(OIDServiceConfiguration *)serviceConfiguration requestParameters:(TokenRequestParameters *)requestParameters result:(FlutterResult)result {
    ...
    [OIDAuthorizationService performTokenRequest:tokenRequest
                                        callback:^(OIDTokenResponse *_Nullable response,
                                                NSError *_Nullable error) {
                                            if (response) {
                                                result([self processResponses:response authResponse:nil]);
                                            } else {
